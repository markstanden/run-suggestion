...
public class UserRepositoryTests
{
    ...
    private async Task PreseedDatabase(int userCount = 100, int eventsPerUser = 10)
    {
        foreach (int _ in Enumerable.Range(0, userCount))
        {
            ...
            await _sut.AddRunEventsAsync(userId, RunBaseFakes.CreateDefaultRunEvents(eventsPerUser));
        }
    }
    ...
    {
        // Arrange
        IEnumerable<RunEvent> runEvents = RunBaseFakes.CreateDefaultRunEvents(eventQty);
        int userId = await _sut.CreateUserAsync(EntraIdFakes.CreateEntraId());
    ...
    {
        // Arrange
        List<RunEvent> runEvents = RunBaseFakes.CreateDefaultRunEvents(runEventQty).ToList();
        int userId = await _sut.CreateUserAsync(EntraIdFakes.CreateEntraId());
        await _sut.AddRunEventsAsync(userId, runEvents);
    ...
    {
        // Arrange
        IEnumerable<RunEvent> runEvents = RunBaseFakes.CreateDefaultRunEvents(runEventQty);
        int userId = await _sut.CreateUserAsync(EntraIdFakes.CreateEntraId());
        await _sut.AddRunEventsAsync(userId, runEvents);
    ...
    public async Task GetUserDataByEntraIdAsync_WithValidEntraId_ReturnsExpectedUserIdData(string entraId)
    {
        // Arrange
        int userId = await _sut.CreateUserAsync(entraId);
        List<RunEvent> runEvents = RunBaseFakes.CreateDefaultRunEvents(3).ToList();
        await _sut.AddRunEventsAsync(userId, runEvents);
        ...
        int validEventQty, int nullEventQty)
    {
        ...
        IEnumerable<RunEvent> validEvents = RunBaseFakes.CreateDefaultRunEvents(validEventQty);
    ...
    public async Task GetRunEventsByUserIdAsync_WithMultipleEvents_ReturnsAllEvents(int eventQty)
    {
        // Arrange
        int userId = await _sut.CreateUserAsync(EntraIdFakes.CreateEntraId());
        IEnumerable<RunEvent> events = RunBaseFakes.CreateDefaultRunEvents(eventQty);
        ...
        int userCount, int eventsPerUser)
    {
        ...
        foreach (int _ in Enumerable.Range(0, userCount))
        {
            ...
            IEnumerable<RunEvent> events = RunBaseFakes.CreateDefaultRunEvents(eventsPerUser);
            await _sut.AddRunEventsAsync(userId, events);
        }
        ...
        int eventsPerInsertion)
    {
        ...
        {
            IEnumerable<RunEvent> events = RunBaseFakes.CreateDefaultRunEvents(eventsPerInsertion);
            await _sut.AddRunEventsAsync(userId, events);
        }
